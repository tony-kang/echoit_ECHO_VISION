...
-- 변경 사항:
--   - 고정 칼럼만 유지: id, year, month, notes, created_at, updated_at, excel_file_id, sales_code
--   - 나머지 모든 칼럼 삭제
...
CREATE TABLE public.ev_sales (
    ...
    -- 엑셀 파일 데이터 (JSONB)
    -- sales_code에 매칭된 엑셀의 모든 칼럼 데이터를 JSON 형태로 저장
    -- 예: {"칼럼명1": "값1", "칼럼명2": "값2", ...}
...
COMMENT ON COLUMN public.ev_sales.excel_file_data IS '엑셀 파일 데이터 (JSONB) - sales_code에 매칭된 모든 칼럼 데이터';
