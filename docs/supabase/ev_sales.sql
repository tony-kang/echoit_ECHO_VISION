...
-- 인덱스 생성
-- 코드 조회 성능 향상
CREATE INDEX IF NOT EXISTS idx_ev_org_code ON public.ev_sales(code);
-- 연도/월 조회 성능 향상
...
-- 코드별 연도/월 조회 성능 향상
CREATE INDEX IF NOT EXISTS idx_ev_org_code_year_month ON public.ev_sales(code, year, month);
