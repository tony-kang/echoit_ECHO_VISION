<script>
	...
	import AmountCodeFormModal from './AmountCodeFormModal.svelte';
	...
	function handleCheckboxChange(value, checked) {
		...
		selectedItems = new SvelteSet(selectedItems); // 반응성 유지
	}
	...
	function handleAddSelectedItems(event) {
		...
		if (itemsToAdd.length > 0) {
			...
			selectedItems = new SvelteSet(); // 반응성 유지를 위해 새 Set 생성
			itemSearchQuery = '';
			showItemDropdown = false;
		}
	}
	...
	function handleSelectAll(selectAll) {
		if (selectAll) {
			...
			selectedItems = new SvelteSet(availableItems);
		} else {
			selectedItems.clear();
		}
		selectedItems = new SvelteSet(selectedItems); // 반응성 유지
...
<MainContent>
					...
					<tr>
						...
						<td>
							<div class="flex flex-wrap gap-1">
								{#each (evCode.items || []).slice(0, 3) as item (item)}
	...
	<AmountCodeFormModal
		open={showFormModal}
		editingEvCode={editingEvCode}
		bind:formCode
		bind:formCategory
		bind:formTitle
		bind:formItems
		bind:formComment
		bind:formDisplayOrder
		bind:newItemValue
		bind:itemSearchQuery
		bind:showItemDropdown
		selectedItems={selectedItems}
		filteredTopLevelEnvCodes={filteredTopLevelEnvCodes}
		...
		onItemSearchInput={handleItemSearchInput}
	/>
